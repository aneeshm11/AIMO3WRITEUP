# !code /data/aneesh/env_vllm4/lib/python3.10/site-packages/vllm/model_executor/models/gpt_oss.py
# vllm '0.19.0'
...
class MLPBlock(torch.nn.Module):
    def __init__(
        ...
        # Support per-layer expert counts for pruned models.
        # Falls back to the global num_local_experts for base models.
        if (
            hasattr(config, "num_local_experts_per_layer")
            and config.num_local_experts_per_layer is not None
        ):
            self.num_experts = config.num_local_experts_per_layer[layer_idx]
        else:
            self.num_experts = config.num_local_experts

        self.router = GateLinear(
            config.hidden_size,
            # The router always needs to project to the *global* expert count
            # so that logits are comparable across layers; unused expert slots
            # are simply never selected by top-k.
...
class GptOssModel(nn.Module, EagleModelMixin):
    ...
    # ------------------------------------------------------------------
    # Helper: resolve the expert count for a single layer.
    # Pruned models supply a per-layer list; base models have a scalar.
    # ------------------------------------------------------------------
    def _get_num_experts_for_layer(self, layer_idx: int) -> int:
        if (
            hasattr(self.config, "num_local_experts_per_layer")
            and self.config.num_local_experts_per_layer is not None
        ):
            return self.config.num_local_experts_per_layer[layer_idx]
        return self.config.num_local_experts

    # ------------------------------------------------------------------
    # Helper: compute EP slicing bounds for a specific layer.
    # Each layer may have a different expert count after pruning.
    # ------------------------------------------------------------------
    def _get_ep_bounds_for_layer(
        self, layer_idx: int, ep_rank: int, ep_size: int
    ) -> tuple[int, int]:
        num_experts = self._get_num_experts_for_layer(layer_idx)
        experts_per_rank = num_experts // ep_size
        ep_rank_start = ep_rank * experts_per_rank
        ep_rank_end = (ep_rank + 1) * experts_per_rank
        return ep_rank_start, ep_rank_end

    # ------------------------------------------------------------------
    # Helper: load a weight that may be smaller than its parameter due
    # to expert pruning (e.g. router bias for a layer with fewer experts).
    # Fills un-populated slots with the dtype's most-negative value so
    # those expert logits are never selected by top-k.
    # Returns True when a pruned-model load was performed; the caller
    # should then skip the normal weight_loader path.
    # ------------------------------------------------------------------
    def _load_weight_with_pruning_support(
        self,
        name: str,
        ...
        if weight.shape == param.data.shape:
            return False  # shapes match – normal path

        if (
            name.endswith(".mlp.router.bias")
            and weight.dim() == 1
            and weight.size(0) < param.data.size(0)
        ):
            # Missing experts should never be selected by router top-k.
            fill_val = (
                torch.finfo(param.data.dtype).min
                if param.data.is_floating_point()
                else torch.iinfo(param.data.dtype).min)
            param.data.fill_(fill_val)
            param.data[: weight.size(0)].copy_(weight)
            return True

        if (
            name.endswith(".mlp.router.weight")
            and weight.dim() == 2
            and weight.size(1) == param.data.size(1)
            and weight.size(0) < param.data.size(0)
        ):
            # Keep surviving experts in the leading rows and zero-fill pruned rows.
            param.data.zero_()
            param.data[: weight.size(0), :].copy_(weight)
            return True
    ...
    def get_expert_mapping(self) -> list[tuple[str, str, int, str]]:
        ...
        #
        # For pruned models each layer may have a different expert count, so we
        # iterate over all layers and collect mappings using each layer's actual
        # expert count.  For base models the list is uniform and this is
        # equivalent to the original single call.
        all_mappings: list[tuple[str, str, int, str]] = []
        seen: set[tuple[str, str, int, str]] = set()
        for layer_idx in range(self.config.num_hidden_layers):
            num_experts = self._get_num_experts_for_layer(layer_idx)
            layer_mappings = FusedMoE.make_expert_params_mapping(
                self,
    ...
    def _load_weights_mxfp4(
        ...
        for name, weight in weights:
            ...
            # Derive per-layer expert count and EP bounds from the weight name.
            layer_id = self._extract_layer_id(name)
            num_experts = self._get_num_experts_for_layer(layer_id) if layer_id is not None else self.config.num_local_experts
            ep_rank_start, ep_rank_end = self._get_ep_bounds_for_layer(layer_id, ep_rank, ep_size) if layer_id is not None else (0, num_experts)
            ...
            for param_name, weight_name, shard_id in stacked_params_mapping:
                ...
                param = params_dict[name]
                weight_loader = getattr(param, "weight_loader", default_weight_loader)
                # Pruned models: router bias may have fewer entries than the
                # parameter (which is sized to the global expert count).
                if not self._load_weight_with_pruning_support(
                    name, param, weight, weight_loader
                ):
                    weight_loader(param, weight)
            loaded_params.add(name)
    ...
    def _load_weights_quark(
        ...
        for name, loaded_weight in weights:
            ...
            # Resolve per-layer expert count and EP bounds.
            if layer_id is not None:
                num_experts = self._get_num_experts_for_layer(layer_id)
                ep_rank_start, ep_rank_end = self._get_ep_bounds_for_layer(
                    layer_id, ep_rank, ep_size
                )
            else:
                num_experts = self.config.num_local_experts
                ep_rank_start = ep_rank * (num_experts // ep_size)
                ep_rank_end = (ep_rank + 1) * (num_experts // ep_size)
            ...
            for param_name, weight_name, shard_id in stacked_params_mapping:
                ...
                for mapping in expert_params_mapping:
                    ...
                    # Pruned models: router bias may have fewer entries than
                    # the parameter (sized to the global expert count).
                    if not self._load_weight_with_pruning_support(
                        param, loaded_weight, weight_loader
                    ):
    ...
    def _load_weights_other(
        ...
        for name, weight in weights:
            ...
            # Resolve per-layer EP bounds from the weight name.
            layer_id = self._extract_layer_id(name)
            ep_rank_start, ep_rank_end = self._get_ep_bounds_for_layer(
                layer_id, ep_rank, ep_size
            ) if layer_id is not None else (
                ep_rank * (self.config.num_local_experts // ep_size),
                (ep_rank + 1) * (self.config.num_local_experts // ep_size),
            )
    ...
    def _extract_layer_id(name: str) -> int | None:
        """Extract the transformer layer index from a weight name.

        Weight names follow the pattern ``...layers.<layer_idx>...``.
        Returns ``None`` if no layer index can be determined.
        """
        parts = name.split(".")
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        # Pass ep_rank / ep_size down to the per-method loaders so that each
        # loader can compute correct per-layer EP bounds independently.
